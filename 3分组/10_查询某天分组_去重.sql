...
BEGIN
    ...
    SET @createTempTable = CONCAT('CREATE TEMPORARY TABLE ', tempTableName, ' AS 
        SELECT callere164, calleee164 FROM ', tableName);
    
    -- 添加WHERE条件（如果有）
    IF whereCondition != '' THEN
        SET @createTempTable = CONCAT(@createTempTable, ' WHERE ', whereCondition);
    END IF;
...
CALL GetDistinctCallDataGrouped('e_cdr_20250316', 'holdtime <= 0');
