...
-- 4.2 判断callere164 主叫号码分组数量是否小于50000条，如果小于50000条，则与其他小于50000条的分组数据合并后，分批导出到csv文件中，默认每个文件最大数量5万条，文件命名为：序号-FFFF-当前服务-当前数据库表名的后6位-月份.日期.csv
...
-- =========================================================
-- 2. 创建底层分批导出过程 (支持动态命名: 序号-FFFF-当前服务-表名后6位-日期)
-- =========================================================
DELIMITER //
...
BEGIN
	...
	IF total_small_groups > 0 THEN
        ...
        -- 向下传递 table_name
		CALL ExportBatchData_Grouped ( 'temp_small_groups_data', export_path, '', 'FFFF', table_name ); 
