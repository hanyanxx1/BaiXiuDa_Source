...
layui.define(['laytpl', 'layer'], function(exports){
  ...
  var tokenName = window.sessionStorage.getItem("name");
  //清除 token，并跳转到登入页
  view.exit = function(){
    let data =  layui.data('layuiAdmin');
    delete data[tokenName];
    window.localStorage.setItem("layuiAdmin",JSON.stringify(data))
    window.sessionStorage.removeItem("load");
    //清空本地记录的 token
    // if(layui.data('layuiAdmin').super_admin_name != 'admin') {
  ...
  view.req = function(options){
    ...
    if(layui.data('layuiAdmin')[name]!=undefined){
      access_token=layui.data('layuiAdmin')[name]["access_token"]
    }
