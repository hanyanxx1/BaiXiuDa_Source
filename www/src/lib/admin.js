...
layui.define(['view', 'table', 'upload'], function (exports) {
  ...
  router = layui.router(),
    ...
    , admin = {
      ...
      correctRouter: function (href) {
        ...
        if (href!=="/plat/plat_info") {
          var pi = window.sessionStorage.getItem("platInfo_interval");
          if (pi && pi > 0) {
            clearInterval(pi);
            window.sessionStorage.setItem("platInfo_interval",0);
          }
        }
        if (href == "" || href == "user/login" || href == "user/password") {
        ...
        if (menu == undefined) {
          var super_own = window.sessionStorage.getItem("own");
          var own = "null";
          if (super_own == "super_admin") {
            own = "super_admin";
          }
          admin.req({
            type: 'get',
            url: layui.setter.configUrl + "/menu/get?own=" + own,
            dataType: "json",
            async: false,
            success: function (obj) {
              var jump;
              if (obj.data != "") {
                jump = obj.data[0].list[0].jump;
                var menus = [];
                for (var i = 0; i < obj.data.length; i++) {
                  var menuList = obj.data[i].list;
                  for (let j = 0; j < menuList.length; j++) {
                    menus.push(menuList[j].jump);
                  }
                }
                menu = menus;
              }
        ...
        if (menu != undefined) {
          ...
          if (menu.includes(href)) {
            ...
            //console.log("不存在,强制跳转");

            //let menuSir = menu.split(",");
            location.hash = search.redirect ? decodeURIComponent(search
              .redirect) : '/' + menu[0];
          }
        } else {
          if (!/^\//.test(href)) href = '/' + href;
  ...
  ! function () {
    ...
    if (request.tokenName) {
      var obj = {};
      obj[request.tokenName] = access_token2 || ''
